...
void check_mouse(XEvent *e, Game *g)
{
	...
	if (savex != e->xbutton.x || savey != e->xbutton.y) {
		...
		if (xdiff > 0) {
			...
			g->astronaut.angle += 0.05f * (float)xdiff;
		...
		if (ydiff > 0) {
			...
			Flt xdir = cos(rad);
			Flt ydir = sin(rad);
			g->astronaut.vel[0] += xdir * (float)ydiff * 0.01f;
			g->astronaut.vel[1] += ydir * (float)ydiff * 0.01f;
...
void physics(Game *g)
{
	...
	else if (g->astronaut.pos[0] > (float)xres) {
		g->astronaut.pos[0] -= (float)xres;
	}
	else if (g->astronaut.pos[1] < 0.0) {
		g->astronaut.pos[1] += (float)yres;
	}
	else if (g->astronaut.pos[1] > (float)yres) {
		g->astronaut.pos[1] -= (float)yres;
	}
	//
	//
